...
#	if defined(SKIN) && defined(CS_SKIN)
	...
	skinSurfaceProperties.Curvature = Skin::CalculateCurvature(worldNormal.xyz);
...
#				if defined(RIM_LIGHTING)
		lightDiffuseColor += lightColor * GetRimLightMultiplier(normalizedLightDirection, viewDirection, worldNormal.xyz) * rimSoftLightColor.xyz * Color::VanillaDiffuseMult();
#				endif  // RIM_LIGHTING

#				if defined(BACK_LIGHTING)
		lightDiffuseColor += lightColor * saturate(-lightAngle) * backLightColor.xyz * Color::VanillaDiffuseMult();
#				endif  // BACK_LIGHTING
#				if defined(HAIR) && defined(CS_HAIR)
		if (SharedData::hairSpecularSettings.Enabled) {
...
#		endif
	...
	{
		float3 glowColor = Color::ColorToLinear(TexGlowSampler.Sample(SampGlowSampler, uv).xyz);
		emitColor *= glowColor;
	}
#	endif
...
#	if defined(IBL)
	if (SharedData::iblSettings.EnableDiffuseIBL) {
		...
		} else if (!SharedData::InInterior || SharedData::iblSettings.EnableInterior || SharedData::iblSettings.EnableInterior) {
